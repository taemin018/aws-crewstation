const giftReportLayout = (() => {
    const giftReport = document.querySelector(".table-reports tbody");
    const showReportGiftList = (reportGift) => {
        reportGift.forEach((gift) => {
            const reportTr = document.createElement("tr");
            reportTr.dataset.reportId = gift.reportId;
            reportTr.dataset.status = gift.processStatus || "PENDING";

            const statusValue = (gift.processStatus || "").toUpperCase();
            ...
            giftReport.appendChild(reportTr);

        })
    }

    return { showReportGiftList: showReportGiftList };
})();